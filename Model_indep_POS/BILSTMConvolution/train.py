...
def prepareData(conlluFileName,dicoCharVocab, dicoVocab, dicoUpos) :
    data_file = open(conlluFileName, "r", encoding="utf-8")
    LX = []
    LY = []
    LC=[]
...
def train(LX, LY,LC, model, nb_iter):
    ...
    for iteration in range(nb_iter):
        ...
        for (x,y,c) in zip(LX,LY,LC):
            ...
            #print( "y ", len(yprime) , " out ", len(output_vec))
...
def main():
    ...
    LX, LY ,LC= prepareData(conlluFileName, dicoCharVocab,dicoVocab, dicoUpos)
